...
playlist_add = select_playlists(sp, username)
...
for artist_id in artists:
    ...
    for playlist in playlist_add:
        ...
        new_playlist_tracks = get_playlist_tracks(sp, playlist)
        new_playlist_tracks = select_artist_tracks(new_playlist_tracks, artist_id)
        for track in new_playlist_tracks:
            if duplicate_track(track, current_names):
                continue
            else:
                count+=1
                add_ids.append(track['id'])
                current_names.append(track['name'])
    if count<min_tracks:
        artist_top10_tracks = sp.artist_top_tracks(artist_id,country='US')
        artist_top10_tracks = artist_top10_tracks['tracks']
        for track in artist_top10_tracks:
